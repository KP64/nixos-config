...
let
  ...
  searxngPort = config.services.searx.settings.server.port;
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    networking.firewall.allowedTCPPorts = [ searxngPort ];

    services = {
      traefik.dynamicConfigOptions.http = {
        routers.searxng = {
          rule = "Host(`searxng.${config.homelab.domain}`)";
          service = "searxng";
        };
        services.searxng.loadBalancer.servers = [
          { url = "http://localhost:${toString searxngPort}"; }
        ];
      };
